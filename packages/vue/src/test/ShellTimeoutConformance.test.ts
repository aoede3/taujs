...
// Shared conformance vector, run in THIS renderer's own environment (jsdom) rather than through a
...
const build = (shellTimeoutMs: unknown) => createRenderer({ appComponent: () => h('div'), headContent: () => '', streamOptions: { shellTimeoutMs } as never });

describe('shellTimeoutMs conformance (@taujs/vue)', () => {
