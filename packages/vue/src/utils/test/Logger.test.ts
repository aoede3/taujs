...
import { createUILogger, createVueErrorHandler } from '../Logger';
...
describe('createUILogger - Server-logger-shaped input', () => {
  ...
  it('uses debug(category, msg) when isDebugEnabled(category) is true', () => {
    const info = vi.fn();
    const debug = vi.fn();
    const isDebugEnabled = vi.fn().mockImplementation((cat) => cat === 'ssr');
    ...
    expect(debug).toHaveBeenCalledWith('ssr', undefined, 'hello');
    expect(info).not.toHaveBeenCalled();
  });
...
// Vue-specific addition to Logger.ts (the byte that diverges from react's upstream copy).
describe('createVueErrorHandler', () => {
  it('logs err, info, and the resolved component name via logger.error (enableDebug)', () => {
    const error = vi.fn();
    const handler = createVueErrorHandler({ error }, true);
    const err = new Error('boom');

    handler(err, { type: { name: 'MyComp' } }, 'render function');

    expect(error).toHaveBeenCalledTimes(1);
    expect(error).toHaveBeenCalledWith('Vue error', { err, info: 'render function', component: 'MyComp' });
  });

  it('falls back to $options.name for the component name', () => {
    const error = vi.fn();
    const handler = createVueErrorHandler({ error }, true);

    handler(new Error('x'), { $options: { name: 'OptComp' } }, 'setup');

    expect(error).toHaveBeenCalledWith('Vue error', expect.objectContaining({ component: 'OptComp', info: 'setup' }));
  });

  it('leaves component undefined when neither name is present', () => {
    const error = vi.fn();
    const handler = createVueErrorHandler({ error }, true);

    handler(new Error('x'), {}, 'watcher');
  ...
  it('handles a nullish instance without throwing', () => {
    ...
    expect(() => handler(new Error('x'), null, 'root')).not.toThrow();
    expect(error).toHaveBeenCalledWith('Vue error', expect.objectContaining({ component: undefined }));
  });
});
