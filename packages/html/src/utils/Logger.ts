/**
 * Copied from the non-Vue parts of `packages/vue/src/utils/Logger.ts` at `73f2867` (drops
 * `createVueErrorHandler`, which has no analogue here - `@taujs/html` renders no component tree and
 * has no framework error channel).
 */
...
export function createUILogger(logger?: LoggerLike, opts: Opts = {}): UILogger {
  ...
  // so production consumers still see renderer warnings/errors.
