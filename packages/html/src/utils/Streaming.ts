/**
 * Copied from `packages/vue/src/utils/Streaming.ts` at `73f2867`, unchanged, PLUS `MAX_NATIVE_TIMEOUT`,
 * `assertTimeout` and `describeTimeout` folded in from `packages/vue/src/SSRRender.ts:222-234` (vue
 * declares them locally inside `createRenderer`; the html implementation contract asks for them here so
 * both the factory and the per-call `renderStream` site can share one copy).
 */
...
export function isBenignStreamErr(err: unknown, source: StreamErrSource): boolean {
  // Render/data-origin errors are never benign by shape: a handler can throw
...
export function startShellTimer(ms: number, onTimeout: () => void): () => void {
  ...
  // and a raw `setTimeout` would clamp them to 1ms and fire almost immediately instead.
 ...
 * `String()`. Same idiom as `packages/renderer-conformance/shellTimeout.ts`'s `describeValue`, so
 * `'10'` (which COERCES to a ten-millisecond delay) is never indistinguishable from `10`.
 */
export const describeTimeout = (value: unknown): string => (typeof value === 'string' ? `'${value}'` : String(value));

/**
 * Validate a `shellTimeoutMs`-shaped option against the shared conformance vector's contract:
 * `0`/`Infinity` are sentinels (no bound), any other value must be a positive finite number no
 * greater than `MAX_NATIVE_TIMEOUT`. `site` names the call site in the thrown message (`createRenderer`
 * at the factory, `renderStream` for the per-call override).
 */
export const assertTimeout = (value: unknown, name: string, site = 'createRenderer'): void => {
...
export function createStreamController(writable: Writable, logger: StreamLogger): StreamController {
  ...
  // must not hand out a footgun promise.
  settle.done.catch(() => {});
  ...
  return {
    ...
    complete(message?: string) {
      ...
      // break the control-flow path it observes.
      try {
