...
`@vue/server-renderer` must be installed at the **same version** as `vue`. A version skew between the two is unsupported and produces subtle SSR/hydration failures. Vite, TypeScript and `@vitejs/plugin-vue` are optional for the root `@taujs/vue` runtime entry, but required when you consume `@taujs/vue/renderer` or `@taujs/vue/plugin` - the renderer's option types are derived from the plugin's own declarations.
</Aside>
