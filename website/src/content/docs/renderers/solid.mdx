...
A lightweight, Solid SSR library with streaming capabilities, built for modern TypeScript applications. Designed as part of the taujs (τjs) ecosystem but fully standalone and runtime-agnostic.
...
**App-owned async** is work your component starts itself - a `createResource` that fetches or computes after the shell. This is what Solid's deferred patch channel exists for, and on a `streaming` route it is what actually streams:
...
<Aside type="caution" title="Route data does not stream">
Under the snapshot bridge, route data travels in `window.__INITIAL_DATA__` and is **never** streamed through Solid's `$df` patch channel (see [The snapshot data bridge](#the-snapshot-data-bridge)). A `streaming` route whose page reads only route data therefore emits no deferred-patch machinery - there is nothing to defer. Streaming is only observable end to end when the application itself owns some async work, so put slow work in an app-owned `createResource` under `<Suspense>`.
</Aside>
...
<Aside type="note" title="cspNonce lives in opts, not a positional argument">
Unlike `@taujs/vue`, whose `renderStream` takes `cspNonce` as a positional parameter, `@taujs/solid` carries it on the final `opts` object (`RenderOptions.cspNonce`) for both `renderSSR` and `renderStream`. When present it nonces the hydration bootstrap and every resource script the renderer emits. See [Hydration and CSP](#hydration-and-csp).
</Aside>
...
`window.__INITIAL_DATA__` is the **single authority** for route data. The server commits the resolved route data into the store before rendering and serialises the same payload into `__INITIAL_DATA__`; the client seeds the store from it during hydration. Route data therefore never travels through Solid's `$df` deferred-patch channel, on any strategy.
...
- **`streaming` routes:** route data is already in `__INITIAL_DATA__`, so a page that reads only route data streams no deferred patches. To exercise streaming you need app-owned async (`createResource` under `<Suspense>`) - that, and only that, flows as `$df` patches.
- **The client never re-runs your route loader.** It hydrates from the snapshot; there is exactly one route-data seed per navigation.

For data that changes *after* the initial render, call your own `/api/*` endpoints from the client with a library such as TanStack Solid Query - τjs keeps SSR request-first and adds no client data channel.
...
2. **Streamed routes need app-owned async to stream.** Route data is already in `__INITIAL_DATA__`, so put slow work in a `createResource` under `<Suspense>` if you want deferred patches.
...
8. **Client-side updates after SSR are userland.** For data that changes after the initial render, call your own `/api/*` endpoints with a client library such as TanStack Solid Query - τjs keeps SSR request-first and does not add a client data channel.
