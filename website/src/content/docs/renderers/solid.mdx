...
τjs governs the response for a URL. What a _subsequent_ navigation does is renderer-independent: a navigation between τjs apps is a full document load and a new governed response, while within one application the app's own client router (Solid Router, TanStack Router or a custom router) owns the transition and any data it needs. See [Navigation boundaries](/guides/micro-frontend/#navigation-boundaries) for the model, and [App Shell Architecture](/guides/app-shell-pattern/) when one shell composes several sections client-side.

For data that changes after the first render, fetch it from your own `/api/*` endpoints with a client library such as TanStack Solid Query. τjs keeps SSR request-first and adds no generic client data channel; the initial snapshot stays the authority for the response it was built for.
---
