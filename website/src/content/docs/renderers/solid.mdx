...
- **Within a single app shell:** the application's own client-side router (Solid Router, TanStack Router, or a custom router) owns navigation and any subsequent data fetching. τjs delivers the governed first response; the shell drives client-side transitions from there. See [App Shell Architecture](/guides/app-shell-pattern/).
