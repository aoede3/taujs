...
**τjs resolves the request, route data and policy; Solid's compiled UI turns that resolved application into an interactive response.**

`@taujs/solid` is the Solid renderer for τjs. τjs matches the URL, runs the route's data loader, and applies its policy (render strategy, hydration, CSP); it then hands the resolved response to Solid, which renders it on the server and hydrates it on the client. You write Solid components and a route table; τjs governs the URL-to-response path around them.

```text
request
  │
  ▼
τjs      match route → run the route data loader → apply policy (strategy, hydrate, CSP)
  │      → assemble the document (head, __INITIAL_DATA__, client bootstrap)
  ▼
Solid    render the resolved app (SSR or streaming) → hydrate on the client
  │
  ▼
response
```

That division is the point. τjs owns request resolution and response assembly; Solid owns the compiled UI and its hydration. It is not a general SSR library you wire yourself - it is the Solid seat inside τjs's governed request/response model. `createRenderer` and `hydrateApp` are also usable standalone against any Node HTTP framework without `@taujs/server`.
...
- **HTTP-framework-independent on Node, with portable static SSR** - the streaming API writes to a Node `Writable`, so it runs under Fastify, Express or any Node HTTP server. Static SSR (`renderSSR`) returns strings and carries no stream dependency, so it also runs on edge runtimes. (It is not a fully runtime-agnostic Web Streams surface; streaming is Node-`Writable`-based.)

`@taujs/solid` mirrors the *contract and capability* of `@taujs/react` and `@taujs/vue` - the same render surface, the same server behaviour, the same hydration beacon events - but it is Solid-native. Route data flows through a signal store you read like any other Solid accessor, and app-owned async uses Solid's own `createResource` + `<Suspense>` rather than a bespoke composable. The public author surface is deliberately small; where it diverges from the other renderers the divergence is called out in place.
...
`onAllReady` is **two-latch**: it fires once *and only once*, after the route data is ready **and** Solid has finished rendering. A fatal, incomplete delivery calls neither `onAllReady` nor `onShellReady`, so it never seeds a truncated document. The resolved route data is delivered to `onAllReady` and serialised into `window.__INITIAL_DATA__`.
...
<Aside type="note" title="The hydrate surface is intentionally small">
`@taujs/react` and `@taujs/vue` carry `logger`, `enableDebug`, `dataKey`, `onStart` and `onSuccess` on their hydrate options. `@taujs/solid` deliberately does **not** inherit them by analogy - the four members above are the whole frozen surface. Adding any of the others would be a public-API change, not a convenience.
</Aside>
...
- If `window.__INITIAL_DATA__` is missing: clears any stale markup and mounts a fresh CSR app. **No hydration callbacks or beacon events fire** - a CSR mount is not a hydration.
...
Creates a store for SSR data. Accepts synchronous data, a promise, or a lazy promise factory:
...
const store3 = createSSRStore(() => Promise.resolve({ c: 3 })); // lazy factory
```
...
A route loader that rejects **before** the shell is a pre-shell fatal: it aborts the render and routes to `onError`, and never seeds a truncated document.
...
`__proto__` in a route-data payload is handled safely end to end: it arrives as an **own** property of `__INITIAL_DATA__` with `Object.prototype` left untouched, so a malicious key cannot pollute the global prototype.
...
Static SSR (no streaming), for edge runtimes or simple cases - `renderSSR` returns the head and app HTML; assemble the document and inject `window.__INITIAL_DATA__` yourself:
...
4. **Head data: `meta` for static, `attr.head` for dynamic.** In streaming mode the route-data snapshot is usually pending when the head is built - guard optional `data` fields, and escape every interpolated value with `escapeHtml`. The renderer appends the hydration bootstrap itself; do not emit it from `headContent`.
...
6. **Handle CSR fallback.** Provide `onHydrationError`; remember the CSR path (no `__INITIAL_DATA__`) emits no hydration callbacks or beacon by design, and a hydration failure reports and stops rather than silently remounting.
...
- `@taujs/vue` - the Vue renderer; shares this package's contract and streaming semantics.
